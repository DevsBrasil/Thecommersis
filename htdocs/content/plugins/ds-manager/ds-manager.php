...
/**
 * Plugin Name: Devs Store - Manager
 * Plugin URI: http://store.dev
 ...
 */
defined('DS_MANAGER_TD') ? DS_MANAGER_TD : define('DS_MANAGER_TD', 'plugin-textdomain');
...
$vars = [
    'slug' => 'ds-manager',
    'name' => 'Devs Store Manager',
    'namespace' => 'dev.store.ds_manager',
    'config' => 'dev_store_ds_manager',
];
...
add_action('admin_notices', function () use ($vars) {
    if (!class_exists('\Themosis\Foundation\Application')) {
        printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', DS_MANAGER_TD));
    }
    ...
    if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
        printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('Your application does not handle the following plugin: ', DS_MANAGER_TD), $vars['name']);
    }
});
...
container('action')->add('plugins_loaded', function () use ($vars) {
	 ...
	 */
	load_plugin_textdomain(DS_MANAGER_TD, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
